...
export function InstallationTab({ session, showMessage }: InstallationTabProps) {
  ...
  const authHppContent = `#pragma once
#include <string>
...
private:
  ...
  const authCppContent = `#include "auth.hpp"
#include <Windows.h>
#include <curl/curl.h>
#include <sstream>
#include <iomanip>
...
bool OxcyAuth::register_with_license(const std::string& license_key, const std::string& user, const std::string& pass)
{
  ...
  const mainCppExample = `#include "auth.hpp"
#include <iostream>
#include <Windows.h>

void show_menu()
{
    std::cout << "\\n=== AuthGuard - Login/Register ===" << std::endl;
    std::cout << "1. Login with existing account" << std::endl;
    std::cout << "2. Register new account with license" << std::endl;
...
int main()
{
    ...
    // Step 1: Initialize and verify version
    auth.init();
    ...
    while (!authenticated) {
        ...
        if (option == 1) {
            ...
            if (auth.validate_user(username, password)) {
                ...
                std::cout << "Welcome to " << auth.get_app_name() << std::endl;
                authenticated = true;
            } else {
            ...
            if (auth.register_with_license(license, username, password)) {
                std::cout << "\\n✓ Registration successful!" << std::endl;
                std::cout << "App name: " << auth.get_app_name() << std::endl;
                std::cout << "Owner ID: " << auth.get_owner_id() << std::endl;
                std::cout << "\\nUpdate auth.cpp with these credentials!" << std::endl;
                authenticated = true;
            } else {
    ...
    // Your application code runs here after successful authentication
    std::cout << "\\n=== Application Running ===" << std::endl;
    std::cout << "User authenticated and ready to use!" << std::endl;
    
    return 0;
}`

  const downloadAll = () => {
    downloadFile("auth.hpp", authHppContent)
    setTimeout(() => downloadFile("auth.cpp", authCppContent), 100)
    setTimeout(() => downloadFile("main.cpp", mainCppExample), 200)
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="bg-gradient-to-br from-slate-800/90 to-slate-900/90 rounded-2xl p-6 border border-slate-700/50 shadow-xl backdrop-blur-sm">
        <h3 className="text-xl font-bold text-slate-200 mb-4 flex items-center gap-2">
          <Download className="h-5 w-5 text-blue-400" />
          Download Files
        </h3>
        <p className="text-slate-400 mb-6">
          Download the AuthGuard C++ integration files pre-configured with your credentials.
        </p>

        <div className="grid md:grid-cols-3 gap-4 mb-6">
          <div className="bg-slate-950/50 rounded-xl p-4 border border-slate-700/50 hover:border-blue-500/50 transition-all group">
            <FileCode className="h-8 w-8 text-blue-400 mb-3 group-hover:scale-110 transition-transform" />
            <h4 className="font-bold text-slate-200 mb-2">auth.hpp</h4>
            <p className="text-sm text-slate-400 mb-4">Header file with class declaration</p>
            <Button
              onClick={() => downloadFile("auth.hpp", authHppContent)}
          ...
          <div className="bg-slate-950/50 rounded-xl p-4 border border-slate-700/50 hover:border-green-500/50 transition-all group">
            <FileCode className="h-8 w-8 text-green-400 mb-3 group-hover:scale-110 transition-transform" />
            <h4 className="font-bold text-slate-200 mb-2">auth.cpp</h4>
            <p className="text-sm text-slate-400 mb-4">Implementation with your credentials</p>
            <Button
              onClick={() => downloadFile("auth.cpp", authCppContent)}
          ...
          <div className="bg-slate-950/50 rounded-xl p-4 border border-slate-700/50 hover:border-red-500/50 transition-all group">
            <FileText className="h-8 w-8 text-red-400 mb-3 group-hover:scale-110 transition-transform" />
            <h4 className="font-bold text-slate-200 mb-2">main.cpp</h4>
            <p className="text-sm text-slate-400 mb-4">Example usage implementation</p>
            <Button
              onClick={() => downloadFile("main.cpp", mainCppExample)}
              className="w-full bg-gradient-to-r from-red-600 to-red-700 hover:opacity-90"
              size="sm"
            >
              <Download className="h-4 w-4 mr-2" />
              Download
      ...
      <div className="bg-gradient-to-br from-slate-800/90 to-slate-900/90 rounded-2xl p-6 border border-slate-700/50 shadow-xl backdrop-blur-sm">
        <h3 className="text-xl font-bold text-slate-200 mb-4">Installation Steps</h3>
        <ol className="space-y-3 text-slate-300">
          <li className="flex gap-3">
            ...
            <span>Download all three files (auth.hpp, auth.cpp, main.cpp)</span>
          </li>
          <li className="flex gap-3">
            <span className="flex-shrink-0 w-6 h-6 rounded-full bg-blue-600 text-white flex items-center justify-center text-sm font-bold">
              2
            </span>
            <span>Add them to your C++ project directory</span>
          </li>
          <li className="flex gap-3">
            <span className="flex-shrink-0 w-6 h-6 rounded-full bg-blue-600 text-white flex items-center justify-center text-sm font-bold">
              3
            </span>
            <span>Install libcurl library for network requests</span>
          </li>
          <li className="flex gap-3">
            <span className="flex-shrink-0 w-6 h-6 rounded-full bg-blue-600 text-white flex items-center justify-center text-sm font-bold">
              4
            </span>
            <span>Include "auth.hpp" in your main.cpp file</span>
          </li>
          <li className="flex gap-3">
            <span className="flex-shrink-0 w-6 h-6 rounded-full bg-blue-600 text-white flex items-center justify-center text-sm font-bold">
              5
            </span>
            <span>Initialize auth system with auth.init() before user login</span>
          </li>
        </ol>
      </div>
      ...
      <div className="bg-blue-950/30 border border-blue-800/30 rounded-xl p-4 backdrop-blur-sm">
        <p className="text-blue-300 text-sm leading-relaxed">
          <strong className="text-blue-200">Note:</strong> Your credentials are already configured in auth.cpp. Make
          sure to keep your API server running on http://127.0.0.1:8000
        </p>
      </div>
    </div>
  )
