  ...
  const handleProfileSetupComplete = async (displayName: string, avatarUrl?: string) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        showNotification("Profile completed and access granted!", "success")
        setTimeout(() => {
          router.push("/dashboard")
        }, 1500)
      } else {
        showNotification(data.message || "Failed to complete profile", "error")
      }
    } catch (error) {
      showNotification(`Error: ${error instanceof Error ? error.message : "Unknown error"}`, "error")
    }
          ...
          <div className="space-y-4">
            ...
            >
              <span className="flex items-center justify-center gap-2">
                ...
                {isDiscordLoading ? "Connecting..." : t("login.button")}
              </span>
            </Button>
